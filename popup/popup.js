...
async function loadApiToken() {
  return new Promise((resolve) => {
    console.log('Loading API Token...');
    browser.storage.local.get(['apiToken'], (result) => {
      if (result && result.apiToken) {
        document.getElementById('api-token').value = result.apiToken;
      }
      resolve(result ? result.apiToken : null);
    });
  });
}
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  console.log('Model names loaded');
  checkFormCompletion();
console.log('Form completion checked');
...
browser.runtime.onMessage.addListener((request, sender) => {
  if (sender.id === browser.runtime.id && request.action === 'imageURL') {
    const img = document.getElementById('selected-image');
    img.src = request.imageUrl;
    ImageOriginalWidth = request.originalSize[0];
    ImageOriginalHeight = request.originalSize[1];
      ...
      if (img && model) {
        try {
          ...
          browser.tabs.query({ active: true, currentWindow: true }, (tabs) => {
            browser.tabs.sendMessage(tabs[0].id, { action: 'reloadPage' });
          });
        } catch (error) {
